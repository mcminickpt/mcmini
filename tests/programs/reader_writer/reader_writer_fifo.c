...
int
read_condition(int reader_ticket_number)
{
  return (num_writers - num_writers_waiting) == 0 &&
         reader_ticket_number == next_ticket_to_be_served;
}

int
write_condition(int writer_ticket_number)
{
  return (num_readers - num_readers_waiting) == 0 &&
         (num_writers - num_writers_waiting) == 1 &&
