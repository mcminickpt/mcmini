...
struct volatile_mem_streambuf : public std::streambuf {
 private:
  char *volatile_cache;
 ...
 public:
  volatile_mem_streambuf(
      ...
      : volatile_mem_streambuf(read_write_region.get(),
                               read_write_region.size()) {
    reset();
  }
  volatile_mem_streambuf(volatile void *rw_start, size_t rw_size)
  ...
  ~volatile_mem_streambuf() { delete[] volatile_cache; }
