...
private:
    inline explicit MCGlobalVariable(void *addr, objid_t id) : MCVisibleObject(id), addr(addr) {}

public:
    void * const addr;
    explicit MCGlobalVariable(void *addr): addr(addr) {}
    MCGlobalVariable(const MCGlobalVariable &global) : MCGlobalVariable(global.addr, global.getObjectId()) {}
