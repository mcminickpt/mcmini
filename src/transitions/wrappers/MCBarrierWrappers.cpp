...
int
mc_pthread_barrier_init(pthread_barrier_t *barrier, const pthread_barrierattr_t *attr, unsigned int count)
{
    auto newlyCreatedShadow = MCBarrierShadow(barrier, count);
    thread_post_visible_operation_hit<MCBarrierShadow>(typeid(MCBarrierInit), &newlyCreatedShadow);
...
mc_pthread_barrier_wait(pthread_barrier_t *barrier)
{
    ...
    thread_post_visible_operation_hit<MCBarrierShadow>(typeid(MCBarrierWait), &newlyCreatedShadow);
    thread_await_mc_scheduler();
