                ...
                void *shmData, MCStack *state)
{
  ...
  if (testIfValidMutex || mutexThatExists -> mutexShadow.state ==
    ...
    if (memcmp(&local_mut, &mutex_initializer, sizeof(mutex_initializer)) != 0){
      mcprintf("\n*** ERROR (pthread_mutex_trylock/traceId: %d): The mutex was\n"
               "***     never initialized.\n"
...
MCMutexTrylock::staticCopy() const
{
  ...
  auto mutexCpy = std::static_pointer_cast<MCMutex, MCVisibleObject>(
    this->mutex->copy());
  bool success = this->mutex->isUnlocked();
...
bool
MCMutexTrylock::enabledInState(const MCStack *state) const
{
  return true;
}
...
MCMutexTrylock::toUniqueRep() const
{
  MCTransitionUniqueRep uniqueRep;
  uniqueRep.typeId = MC_MUTEX_TRYLOCK;
...
{
  bool isAcquired = this->mutex->isUnlocked();
  mcprintf("thread %lu: pthread_mutex_trylock(mut:%u) -> %s\n", this->thread->tid,
           countVisibleObjectsOfType(this->mutex->getObjectId()),
           isAcquired ? "SUCCESS" : "FAILED");
}
